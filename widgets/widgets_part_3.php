...
class gm_wid extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo'<section class="section section_wid" '; if(!sm_NullEmpty($main[1])){echo 'id="'.$main[1].'"';} echo' style="background:'.$main[5].'">';
		if(isset($main[9]) && $main[9] !== ''){
			...
			if(isset($instance['in15']) == true && $instance['in15'] == "on"){
				echo'<script>
				(function($){
					function lel(){
		...
		$style = "";
		if(empty($main[3])&&empty($main[0])){
			$style = " style=\"display:none;\" ";
		}
		echo"<div class=\"section-header\" $style>";
					echo'<h2 class="white-text" style="color:'.$main[2].'">'.$main[0].'</h2>';
		if( !empty($main[3]) ): echo'<h6 class="white-text" style="color:'.$main[4].'">'.$main[3].'</h6>';endif;echo'</div>';
		if(sm_NullEmpty($main[12])){$main[12]='Standard';}
		echo'<div id="gmaps" data-maps="'.$main[6].'/'.$main[7].'/'.$main[8].'" data-maps-hue="'.$main[10].'" '; 
		if(isset($main[13]) && !sm_NullEmpty($main[13]) && !sm_NullEmpty($main[14]) && isset($instance['in15']) == true && $instance['in15'] != "on"){echo' style="height:'.$main[13].$main[14].'" ';}
	...
	public function form( $instance ) {
		...
		sm_fieldProtoImageUpload('Background:',$this->get_field_id( 'in5' ),$this->get_field_name( 'in5' ),$main[5], 'Input any Background');
		
		sm_fieldProtoCheckboxDes('Full Height (100vh)',$this->get_field_id( 'in15' ),$this->get_field_name( 'in15' ),$main[15],'This options adjust height to fit within browser window');
		
		sm_fieldProtoSelectUnits('Height:',$this->get_field_id( 'in13' ),$this->get_field_name( 'in13' ),$main[13],$this->get_field_id( 'in14' ),$this->get_field_name( 'in14' ),$main[14]);
		
		echo'<h2>'; _e('Maps Settigs','somnium');echo'</h2>';
		
		sm_fieldProtoDes('Google Maps JavaScript API key',$this->get_field_id( 'in9' ),$this->get_field_name( 'in9' ),$main[9],'More info here: https://developers.google.com/maps/documentation/javascript/get-api-key');
		
		sm_fieldProto('Latitude of center:',$this->get_field_id( 'in6' ),$this->get_field_name( 'in6' ),$main[6]);
		
		sm_fieldProto('Longitude of center:',$this->get_field_id( 'in7' ),$this->get_field_name( 'in7' ),$main[7]);
			
		sm_fieldProtoNumber('Zoom:',$this->get_field_id( 'in8' ),$this->get_field_name( 'in8' ),$main[8]);
		
		sm_fieldProtoColorPicker('Hue of Map:',$this->get_field_id( 'in10' ),$this->get_field_name( 'in10' ),$main[10]);
		
		sm_fieldProtoNumber('Saturation (min -100, max 100):',$this->get_field_id( 'in11' ),$this->get_field_name( 'in11' ),$main[11], 1, -100, 100);
		
		sm_fieldProtoSelection('Map Type',$this->get_field_id( 'in12' ),$this->get_field_name( 'in12' ),$main[12] ,array('Standard','Hybrid','Satellite','Terrain'));
...
class icn_wid extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		sm_callDefault($main[12], 40);
		sm_callDefault($main[13], 'px');
		sm_callDefault($main[4], 40);
		sm_callDefault($main[8], 'px');
		sm_callDefault($main[14], 14);
		sm_callDefault($main[15], 'px');
		$selRes='';
		switch ($main[7]){
			case '8%':
				$selRes = 'col-md-1';
		...
		switch ($main[11]){
			...
			case 'Font Awesome Icon':
				...
				break;
			case 'SVG Icon':
				echo'<img alt="iconWid" class="iconWid svg" '; if(!sm_NullEmpty($main[2])){echo'src="'.$main[2].'"';}else{echo'src="#"';} echo'/>';
				break;
			case 'Image':
				echo'<img alt="iconWid" class="iconWid " '; if(!sm_NullEmpty($main[2])){echo'src="'.$main[2].'"';}else{echo'src="#"';} echo'/>';
				break;
		}	
		echo'<hr class="spinHrSpacer"><div class="underspin"><h1 style="font-size:'.$main[12].$main[13].'">'.$main[0].'</h1><p style="font-size:'.$main[14].$main[15].'">'.$main[1].'</p></div>'.(!sm_NullEmpty($main[16]) ? '</a>' : '').'</div>';		
	}
	...
	public function form( $instance ) {
		...
		sm_fieldProtoSelectUnits('Text Size:',$this->get_field_id( 'in14' ),$this->get_field_name( 'in14' ),$main[14],$this->get_field_id( 'in15' ),$this->get_field_name( 'in15' ),$main[15]);
		
		sm_fieldProto('Link:',$this->get_field_id( 'in16' ),$this->get_field_name( 'in16' ),$main[16]);
			
		sm_fieldProtoIconSelection('Icon Select',$this->get_field_id( 'in11' ),$this->get_field_name( 'in11' ),$main[11],array('Image','SVG Icon','Font Awesome Icon', 'Super Title'));
		...
		sm_fieldProtoImageUpload('Icon:',$this->get_field_id( 'in2' ),$this->get_field_name( 'in2' ),$main[2]);
	
		sm_fieldProtoColorPicker('Color:',$this->get_field_id( 'in9' ),$this->get_field_name( 'in9' ),$main[9]);
		
		sm_fieldProtoIconPicker('Select Icon:',$this->get_field_id( 'in10' ),$this->get_field_name( 'in10' ),$main[10]);
		
		sm_fieldProto('Super Title:',$this->get_field_id( 'in3' ),$this->get_field_name( 'in3' ),$main[3]);
			
		sm_fieldProtoSelectUnits('Size:',$this->get_field_id( 'in4' ),$this->get_field_name( 'in4' ),$main[4],$this->get_field_id( 'in8' ),$this->get_field_name( 'in8' ),$main[8]);
		...
		sm_fieldProtoSelection('Width of This Widget',$this->get_field_id( 'in7' ),$this->get_field_name( 'in7' ),$main[7] ,array('8%','17%','25%','33%','50%','100%'));
	}
...
class qr_wid extends WP_Widget {
			...
			echo'<div class="wid-query col-md-12" data-sr="'.$main[3].'">';
				...
				echo'<div class="col-md-4 col-sm-4 wid-images">';
					...
					while ( $the_query2->have_posts() && $xcnt < $main[1]) {
						...
						echo'<a href="' . get_permalink() . '"><div class="wid-post-dsc"><p>' . sm_field_excerpt(get_the_ID() , get_the_content(), $main[2]) . '</p></div></a></div>';
					$xcnt++;
					}
				echo'</div>
			</div>
	...
	public function form( $instance ) {
		...
		sm_fieldProtoNumberDes('Number of news:',$this->get_field_id( 'in1' ),$this->get_field_name( 'in1' ),$main[1], 'Maximum 8 news', 1, 0, 8);
			
		sm_fieldProtoNumber('Number of Description Excerpt Words:',$this->get_field_id( 'in2' ),$this->get_field_name( 'in2' ),$main[2]);
			
		sm_fieldProtoScrollRevealDes('Scroll Reveal:',$this->get_field_id( 'in3' ),$this->get_field_name( 'in3' ),$main[3],'For Example: enter left and move 300px over 1s after 0.5s');
			
		sm_fieldProtoSelection('Width of This Widget',$this->get_field_id( 'in4' ),$this->get_field_name( 'in4' ),$main[4] ,array('50%','100%'));
		
		sm_fieldProtoCategoryDropdown('Category selection', $this->get_field_name( 'in5' ), $main[5],'name');
