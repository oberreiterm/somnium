...
class WidgetImageField {
    ...
    function __construct( $widget = null, $image_id = 0 ) {
        ...
        if ( $uri && in_array( $file, array( 'widgets.php' ) ) && is_admin() ) {
			...
			wp_enqueue_script( 'admin-widgets',get_admin_url().'/js/widgets.js');
            wp_enqueue_script( 'widgetimagefield', get_template_directory_uri() . '/color_picker/script.js', array( 'jquery', 'jquery-ui-core', 'thickbox', 'media-upload','admin-widgets'), false, true );
			 
            wp_enqueue_style( 'thickbox' );
            wp_enqueue_style( 'widgetimagefield', get_template_directory_uri() . '/color_picker/style.css' );
        }
